...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    strings = [s for s in paragraphs if select(s)]
    if k >= len(strings):
        return ''
    return strings[k]
    # END PROBLEM 1
...
def about(keywords):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def is_keyword(paragraphs):
        paragraphs = split(lower(remove_punctuation(paragraphs)))
        strings = [s for s in keywords if s in paragraphs]
        if strings == []:
            return False
        return True
    return is_keyword
    ...
    if typed_words == []:
        ...
        while k < len(typed_words) and k != len(source_words):
            if typed_words[k] == source_words[k]:
                result += 1
            k += 1
        return result / len(typed_words) * 100 / 1
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        for source_word in word_list:
            n = diff_function(typed_word, source_word, limit)
            if n <= limit:
                words = words + [source_word]
                limits = limits + [n]
        if words == []:
            return typed_word
        else:
...
def final_diff(typed, source, limit):
    ...
    if typed[0] == source[0]:
        match = minimum_mewtations(typed[1:], source[1:], limit)
    else:
        match = limit + 1
    ...
    return min(match, add, remove, substitute)
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    k = 0
    while k < min(len(source),len(typed)) and typed[k] == source[k]:
        k += 1
    upload({'id':user_id,'progress':k / len(source)})
    return k / len(source)
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    while k < len(tpp):
        time_i = []
        for i in range(1,len(tpp[k])):
             time_i = time_i + [tpp[k][i] - tpp[k][i-1]]
        k += 1
        times = times + [time_i]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fast_words = [[] for _ in player_indices]
    
    for x in word_indices:

        for k in player_indices:
            fast_players = [ times[n][x] for n in player_indices]
            if times[k][x] == min(fast_players):
                fast_words[k] = fast_words[k] + [words[x]]
                break
