...
def boar_brawl(player_score, opponent_score):
    ...
    if tens_digit != ones_digit:
        total = 3 * abs(tens_digit - ones_digit)
    else:
        total = 1
    return total
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    score = 0
    opponent_score = 0
    roll1 = strategy(score,opponent_score)
    while score < goal:
        opponent_score = 0
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max = 0
    while n <= 10:
        averaged = make_averaged(roll_dice,times_called)(n,dice)
        if max < averaged:
            max = averaged
            lower = n
        n += 1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    total = sus_points(score+boar) - score
    if total >= threshold:
        num_rolls = 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    n = sus_strategy(score,opponent_score)
    return n  # Remove this line once implemented.
    # END PROBLEM 12
